...
const RightSection = () => {
    ...
    return (
        <div className='md:h-full md:w-3/12 flex w-full flex-col bg-secondary items-center md:py-10 rounded-l-3xl text-dark2 md:gap-3 gap-2 py-5'>

            <div className='flex w-full justify-between px-4 sm:hidden'>
                <p className='text-xl md:text-2xl font-semibold'>{city?.locality}, {city?.countryName}</p>
            ...
            <div className='text-center md:mt-6 flex md:flex-col md:gap-3 items-center justify-between px-3 w-full'>
                <div>
                    <p className='text-7xl md:text-5xl font-bold sm:flex items-start leading-none hidden'>
                        {weather?.current.feels_like}°<span className='text-3xl mt-1'>C</span>
                    </p>
                    <p className='text-7xl font-bold flex items-start leading-none sm:hidden'>
                        {Math.round(weather?.current.temp ?? 0)}°<span className="text-4xl mt-2">C</span>
                    </p>
                    <div className='flex justify-center md:gap-3 md:mt-2'>
                        <IconWind className='w-5 h-5' /> <span>{weather?.current.wind_speed} kmph</span>
                        <IconDroplet className='w-5 h-5' /> <span>{weather?.current.humidity}%</span>
                    </div>
                </div>
                <div>
                    <p className='mt-2 text-lg text-gray-500'>Feels like {Math.round(weather?.current.feels_like ?? 0)}°</p>
                    <p className='text-2xl font-semibold flex items-center justify-center gap-2'>
                        {weather?.current.weather[0].description}
                    </p>
                </div>
            </div>
            <SunProgress sunrise={moment.unix(weather?.current.sunrise ?? 0).format('HHmm')} sunset={moment.unix(weather?.current.sunset ?? 0).format('HHmm')} />
